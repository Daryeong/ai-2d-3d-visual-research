const DATA = {
  prompts: {
    1: ['2D_Canva_Prompt 01.jpg','2D_Codex_Prompt 01.png','2D_Firefly Image 5_Prompt 01.png','2D_Flow_Prompt 01_Nano Banana 2.jpeg','2D_Grok_Prompt 01.jpg','2D_MiriCanvas_Prompt 01_GPT Image 2.png'],
    2: ['2D_Canva_Prompt 02.jpg','2D_Codex_Prompt 02.png','2D_Firefly Image 5_Prompt 02.png','2D_Flow_Prompt 02_Nano Banana 2.jpeg','2D_Grok_Prompt 02.jpg','2D_MiriCanvas_Prompt 02_GPT Image 2.png'],
...
const toolName = (name) => name.replace(/^2D_|^3D_/, '').split('_Prompt')[0].replace(/ Image 5/g,'').replace(/Nano Banana 2/g,'Nano Banana 2').replace(/MiriCanvas/g,'MiriCanvas').replace(/-/g,' ').trim();
...
function render2d(num=1){
  ...
  DATA.prompts[num].forEach(name=>wrap.appendChild(card(`gm_Portfolio_img2d/${name}`, toolName(name), name)));
}
...
function renderFails(){
  ...
  DATA.fails.forEach(name=>wrap.appendChild(card(`gm_Portfolio_img2d/${name}`, `${toolName(name)} / Failed Result`, name)));
}
